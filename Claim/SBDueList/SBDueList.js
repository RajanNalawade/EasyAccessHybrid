...
function validateSBDueListForm() {
    if (sbDueListForm.checkValidity()) {
        getMaturityData();
    }
}
...
function getMaturityData() {
    let body = `<?xml version="1.0" encoding="utf-8"?>
                ...
                <soap:Body>
                    <checkSBProposal xmlns="http://tempuri.org/">
    ...
    showLoader();
    self.ApiCallService('/checkSBProposal', body, '<CIFPolicyList>', '</CIFPolicyList>').then((response) => {
        self.hideLoader();
            ...
            if (response.CIFPolicyList !== undefined) {
                ...
                if (response.ScreenData !== undefined && response.ScreenData !== null) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        var POLICYNUMBER, lifeAssuredName, STATUS, benefitTerm,
                        ...
                        // contactNumber = response[i].Contact_x0020_No._x0028_M_x0029_;

                        // if (contactNumber == undefined) {
                        //     contactNumber = "";
                        // }

                        output += `<table class='table table-striped borderless one' id="tableRow${i}"> <tbody>`;
                        ...
                        output += "<tr> <th class='tableHeader' style='background-color: lightgray;'>SB Due Date  </th> <td> " + MATURITYDATE + "</td></tr>";
                        output += "<tr> <th class='tableHeader' style='background-color: lightgray;'>* SB DUE Amount:  </th> <td> " + PAYMENTAMOUNT + "</td></tr>";
...
function getProposalNo() {
    return document.forms["sbDueListForm"]["proposalNo"].value;
}
